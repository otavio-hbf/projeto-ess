...
When("the user writes the genre {string} in the input text", (input: string) => {
    cy.getDataCy("input_value").type(input).type("{enter}");
})

When("The user clicks in a {string} container", (button: string) => {
    cy.getDataCy(button).should('be.visible')
    .first()
    .click();;
})
...
Then("a list with at most {string} songs is displayed in the {string} container", (maxSongs:string, section:string)=> {
    cy.get(`[data-cy="${section}"]`).should("have.length.at.most", parseInt(maxSongs));
})

Then("a message is displayed saying {string} in the {string} span", (message: string, id:string) => {
