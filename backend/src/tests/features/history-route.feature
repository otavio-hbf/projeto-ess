Feature: History Routes
    ...
    Scenario: Get user history from user id
        Given the HistoryService returns a history for user_id "t1" with 3 items with song_id "t1", "t2" and "t3"
        When I send a GET request to "/user/t1/history"
        Then the response status should be "200"
        And the response JSON should contain a history with 3 items with song_id "t1", "t2" and "t3"

    # GET most played
    # /user/:id/hot
    Scenario: Get most played songs from user id
        Given the user with id "a1" has a history with the following items:
            | times_played | song_id | title                   | artist      | genre | duration |
            | 3            | f1       | Never Gonna Give You Up | Rick Astley | Rock  | 300      |
            | 3            | f2       | Yellow                  | Coldplay    | Pop   | 150      |
            | 1            | f3       | Yellow Submarine        | The Beatles | Rock  | 100      |
            | 2            | f4       | Ticket to Ride          | The Beatles | Rock  | 250      |
        When I send a GET request to "/user/a1/hot"
        Then the response status should be "200"
        And the response JSON should contain a list with the following songs in order:
            | song_id | title                   | artist      | genre | times_played |
            | f1       | Never Gonna Give You Up | Rick Astley | Rock  | 3            |
            | f2       | Yellow                  | Coldplay    | Pop   | 3            |
            | f4       | Ticket to Ride          | The Beatles | Rock  | 2            |
            | f3       | Yellow Submarine        | The Beatles | Rock  | 1            |
